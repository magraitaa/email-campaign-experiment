...
Проект посвящён анализу результатов A/B-теста двух email-кампаний.  
Цель исследования — определить, повышают ли email-рассылки посещаемость сайта, конверсию в покупку и выручку на пользователя по сравнению с отсутствием рассылки.
...
- посещаемость увеличилась на **7,66 процентного пункта**;
- конверсия увеличилась на **0,68 процентного пункта**;
...
- посещаемость увеличилась на **4,52 процентного пункта**;
- конверсия увеличилась на **0,31 процентного пункта**;
...
- В датасете отсутствует уникальный идентификатор пользователя, поэтому полностью одинаковые строки нельзя однозначно считать техническими дубликатами.
- В расчётах используется выручка, а не прибыль.
- Данные не содержат стоимость отправки писем и маржинальность заказов.
- Масштабирование результатов предполагает, что новая аудитория будет похожа на участников эксперимента.
- Исследование выполнено на открытом историческом датасете.
